...
void exNode(nodeType *p,int c, int l,int *ce, int *cm)
{
	...
	s = word;
	switch(p->type) {
		case typeInt: sprintf (word, "c(%d)", p->i.value); break;
		case typeBool: sprintf (word, "c(%d)", p->b.value); break;
		case typeStr: sprintf (word, "c(%s)", p->s.value); break;
	...
	*cm = c + w / 2;
	...
	/* node is leaf */
	if (p->type == typeInt || p->type == typeBool || p->type == typeStr || p->type == typeId || p->opr.nops == 0) 
	{
